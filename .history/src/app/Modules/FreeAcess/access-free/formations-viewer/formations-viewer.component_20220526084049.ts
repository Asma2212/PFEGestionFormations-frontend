...
this.sessionservice.ListFavoris(this.username).subscribe(data =>{
  ...
  console.log("test fav",this.listFav.filter(s => s.idSession == 141))
})
if(this.username)
this.candidatService.getCandidatByUsername(this.username).subscribe(data =>{
  this.candidat = data

console.log("candidat authentifié",this.candidat.sessionFormationList)})
...
return false ;
  ...
  console.log("test fav",this.listFav.includes(sess))
